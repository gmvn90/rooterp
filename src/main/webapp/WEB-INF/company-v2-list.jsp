...
<div style="width: 1400px ! important; padding: 15px; background: none repeat scroll 0% 0% white; height: auto; overflow: hidden; min-height: 400px"
    ...
    <form method="get" action="company-v2-list.htm" id="x-form">
            ...
            <table class="table_filter">
                ...
                <tr>
                    <td width="200px">
                        ...
                        <select name="companyType" id="companyType" size="5" class="filter"
                                style="width: 100%; overflow: auto">
                            <option value="-1" <c:if test="${companyType==-1}">selected</c:if>>All</option>
                            <c:forEach var="item" items="${companyTypes}">
                                <option
                                        <c:if test="${item.id==companyType}">selected</c:if>
        ...
        <input class="right" id="createNew" data-href="company-v2.htm" type="button" value="New Company"/>
        ...
        <div style="width: 100%;overflow: hidden; height: auto; border-bottom: 1px solid #B8C2CC; padding-bottom: 10px">
            ...
            <table class="dataTableStandard" style="width: 100%">
                ...
                <c:forEach var="item" items="${companies}">
                    <tr class="go2Detail" data-href="company-v2.htm?id=${item.id}">
                        <td>${item.fullName}</td>
                        <td>${item.name}</td>
                        <td>${item.address1}</td>
                        <td>${item.representative}</td>
                        <td>${item.representativeRole}</td>
                        <td>${item.taxCode}</td>
                        <td>${item.email}</td>
                        <td>${item.fax}</td>
    ...
    <script>
        ...
        $(document).ready(function () {
            var will_changes = "#companyType,#perPage";
