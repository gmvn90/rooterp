...
<head>
    ...
    <title>Packing Master List</title>
    <style>
        .styleTotal td {
...
<jsp:include page="header.jsp">
    <jsp:param name="url" value="58"/>
    <jsp:param name="page" value="105"/>
</jsp:include>
<div style="width: 700px ! important; padding: 15px; background: none repeat scroll 0% 0% white; height: auto; overflow: hidden; min-height: 400px"
     class="container_16 border" id="wrapper">
    <form method="get" action="packing-master-list.htm" id="x-form">
        <input class="right" id="createNew" data-href="packing-master.htm" type="button" value="New Packing Master"/>


        <div style="width: 100%;overflow: hidden; height: auto; border-bottom: 1px solid #B8C2CC; padding-bottom: 10px">
            <h3>Packing Master list</h3>
            ...
            <table class="dataTableStandard" style="width: 100%">
                ...
                <tr>
                    ...
                    <th>Weight</th>
                    <th>Kg Per Bag</th>
                    <th>Bags Per Pallet</th>
                </tr>
                </thead>
                <tbody>
                <c:forEach var="item" items="${packings}">
                    <tr class="go2Detail" data-href="packing-master.htm?id=${item.id}">
                        <td>${item.name}</td>
                        <td>${item.weight}</td>
                        <td>${item.kgPerBag}</td>
                        <td>${item.bagsPerPallet}</td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
