...
<head>
    ...
    <title>Courier Master List</title>
    <style>
        .styleTotal td {
...
<jsp:include page="header.jsp">
    <jsp:param name="url" value="52"/>
    <jsp:param name="page" value="99"/>
</jsp:include>
<div style="width: 1400px ! important; padding: 15px; background: none repeat scroll 0% 0% white; height: auto; overflow: hidden; min-height: 400px"
    ...
    <form method="get" action="courier-master.htm" id="x-form">
        <input class="right" id="createNew" data-href="courier-master.htm" type="button" value="New Courier Master"/>


        <div style="width: 100%;overflow: hidden; height: auto; border-bottom: 1px solid #B8C2CC; padding-bottom: 10px">
            <h3>Courier Master list</h3>
            ...
            <table class="dataTableStandard" style="width: 100%">
                ...
                <tbody>
                <c:forEach var="item" items="${couriers}">
                    <tr class="go2Detail" data-href="courier-master.htm?id=${item.id}">
                        <td>${item.fullName}</td>
                        <td>${item.name}</td>
                        <td>${item.address1}</td>
                        <td>${item.address2}</td>
